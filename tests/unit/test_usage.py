...
def test_record_usage_computes_cost_from_pricing_table():
    ...
    cost = usage.record_usage("claude-opus-5", input_tokens=1_000_000, output_tokens=1_000_000)

    assert cost == 15.0 + 75.0
    budget = usage.get_job_budget()
    assert budget.total_cost_usd == 90.0
...
def test_is_over_budget_false_below_threshold():
    ...
    usage.record_usage("claude-sonnet-5", input_tokens=1000, output_tokens=1000)
...
def test_record_usage_is_noop_without_active_job():
    usage._current_budget.set(None)
    cost = usage.record_usage("claude-opus-5", input_tokens=1000, output_tokens=1000)
